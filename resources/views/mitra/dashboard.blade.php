...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
    ...
    <x-stat-card title="Draft" :value="$draft" bgColor="bg-gray-100"
    ...
    <x-stat-card title="Diajukan / Review" :value="$diajukan" bgColor="bg-blue-200"
...
<div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
    <x-stat-card title="Ditolak" :value="$ditolak" bgColor="bg-red-100"
    ...
    <x-stat-card title="Disetujui" :value="$disetujui" bgColor="bg-yellow-100"
    ...
    <div class="space-y-3">
        ...
        <div class="py-3 border-b border-gray-100 last:border-0">
            <div class="flex items-center justify-between mb-1">
                ...
                <p class="text-sm font-semibold text-primary">{{ $ks->tanggal_pembahasan_ks?->format('d M Y') }}</p>
            </div>
            <p class="text-xs text-gray-500 mb-1">{{ $ks->tentang }}</p>
            <div class="flex items-center gap-4 text-xs text-gray-400">
                <span>🕐 {{ substr($ks->jam_pembahasan, 0, 5) }}</span>
                <span>📍 {{ $ks->lokasi_pembahasan }}</span>
                @if($ks->link_meeting)
                <a href="{{ $ks->link_meeting }}" target="_blank" class="text-primary hover:underline">🔗 Link Meeting</a>
                @endif
                @if($ks->pic_pembahasan)
                <span>👤 PIC: {{ $ks->pic_pembahasan }}</span>
                @endif
            </div>
        </div>
        @endforeach
    ...
    <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-100">
            ...
            <tr class="hover:bg-gray-50">
                ...
                <td class="px-4 py-2.5"><x-status-badge :status="$ks->status_pengajuan" :label="$ks->status_label" /></td>
