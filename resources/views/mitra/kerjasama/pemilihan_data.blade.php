...
<div class="space-y-6" x-data="pemilihanData()">
    ...
    @if($kerjasama->status_pemilihan_data === 'submitted')
    <div class="bg-amber-50 border border-amber-200 py-2.5 px-4 rounded-xl text-amber-900 text-[11px] flex items-center gap-2.5 shadow-2xs">
                ...
                <div class="flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-slate-200/80 text-xs">
                    <div class="flex items-center gap-3">
                        <button type="button" @click="toggleAllAccordions()" class="inline-flex items-center gap-1.5 text-indigo-600 hover:text-indigo-800 font-medium hover:underline">
                            <template x-if="anyAccordionOpen">
...
function pemilihanData() {
    return {
        ...
        get anyAccordionOpen() {
            return Object.values(this.openTables).some(Boolean);
        },

        toggleAllAccordions() {
            let shouldOpen = !this.anyAccordionOpen;
            let updated = {};
            this.allTables.forEach(t => {
                updated[t.db + '.' + t.name] = shouldOpen;
            });
