        ...
        <header class="bg-white border-b border-gray-200 px-6 py-4 flex justify-between items-center sticky top-0 z-10 shadow-sm">
            ...
            <div class="flex items-center gap-3">
                ...
                <div class="relative" x-data="{ 
                    ...
                    notifications: [],
                    fetchNotifications() {
                        fetch('{{ route('notifications.latest') }}?role=admin')
                            .then(res => res.json())
                            .then(data => {
                    ...
                    <button type="button" 
                            ...
                            class="relative p-2.5 text-gray-500 hover:text-indigo-600 rounded-xl hover:bg-slate-100/80 transition-all duration-150 focus:outline-none"
                    ...
                    <div x-show="open" 
                        ...
                        <div class="px-4 py-3 bg-slate-50/80 border-b border-slate-100 flex items-center justify-between">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-indigo-50 text-indigo-600 border border-indigo-100" x-text="unreadCount + ' Baru'"></span>
                            </div>
                            <button type="button" @click="markAllRead()" class="text-[10px] text-indigo-600 font-semibold hover:underline">
                                Tandai Dibaca
                            </button>
                        </div>
                        ...
                        <div class="divide-y divide-slate-100 max-h-72 overflow-y-auto">
                            <template x-for="item in notifications" :key="item.id">
                                <a :href="item.url || '{{ route('admin.kerjasama.index') }}'" class="px-4 py-3 hover:bg-slate-50/80 flex items-start gap-3 transition-colors group">
                                    <div class="w-2 h-2 rounded-full mt-1.5 shrink-0" :class="item.is_read ? 'bg-slate-300' : 'bg-indigo-500'"></div>
                                    <div class="flex-1">
                                        <p class="font-semibold text-slate-800 group-hover:text-indigo-600 transition-colors text-xs" x-text="item.title"></p>
                        ...
                        <div class="px-4 py-2 bg-slate-50/50 border-t border-slate-100 text-center">
                            <a href="{{ route('admin.kerjasama.index') }}" class="text-[11px] font-semibold text-indigo-600 hover:text-indigo-700 transition-colors">
